...
async function rebuildMDQ() {
  const db = await getDb();
  if (!db) throw new Error('Database not available');

  const slug = 'million-dollar-quartet-scr-debut';
  
  // New cover image - Brandon with scenic model
  const coverImage = 'https://files.manuscdn.com/user_upload_by_module/session_file/310519663337866878/AnobejMscUsupIXj.jpeg';
  
  // New excerpt
  const excerpt = 'Brandon PT Davis brings his narrative-driven design approach to South Coast Repertory in this LORT production of Million Dollar Quartet, co-designing with Efren Delgadillo Jr. a transformative set that captures the birth of rock and roll.';
  ...
  console.log('✓ Million Dollar Quartet article rebuilt!');
  console.log('  - New cover: You with scenic model');
  console.log('  - Co-designer credit: Efren Delgadillo Jr.');
  console.log('  - Full creative team block');
  console.log('  - Scenic model gallery');
  console.log('  - LORT debut emphasis');
