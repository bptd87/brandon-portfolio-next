...
const manusUrlPattern = /https?:\/\/[^\s"'`)\]]*(?:manuscdn\.com|manus-user-assets|manus\.im)[^\s"'`)\]]*/gi;
...
function getTargetPrefix(file: string, kind: MediaKind) {
  ...
  const projectMatch = file.match(/content\/projects\/(?:[^/]+\/)*([^/]+)\//);
  if (projectMatch) {
    const slug = sanitizeSegment(projectMatch[1]);
    return [mediaRoot, "projects", "recovered", slug];
  }

  if (file.endsWith("shared/localArticles.generated.ts")) {
    return [mediaRoot, "migrated", "manus", "generated-articles"];
  }
  ...
  return [mediaRoot, "migrated", "manus", "misc"];
}
...
async function walkFiles(targetPath: string, output: string[]) {
  ...
  const stat = await readdir(abs, { withFileTypes: true }).catch(() => null);
...
async function collectScans() {
  ...
  for (const file of files) {
    ...
    const urls = Array.from(new Set(text.match(manusUrlPattern) || []));
...
async function main() {
  ...
  if (!scans.length) {
    console.log("No Manus asset references found in active scan targets.");
    return;
  }

  console.log(`Found ${allUrls.length} Manus asset URLs across ${scans.length} files.`);
  scans.forEach((scan) => {
  ...
  if (!shouldWrite) {
    ...
    console.log("\nRun with --write and BLOB_READ_WRITE_TOKEN to fetch and upload Manus assets to Blob.");
    return;
  }
  ...
  console.log(`Recovered ${replacements.size} Manus asset URLs into Blob.`);
  if (failed.length) {
