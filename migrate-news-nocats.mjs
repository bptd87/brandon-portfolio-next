...
VALUES ('${slug}', '${title}', '${excerpt}', '${blocksJson}', ${coverUrl ? `'${coverUrl}'` : 'NULL'}, NULL, ${dateStr ? `'${dateStr}'` : 'NULL'}, ${link ? `'${link}'` : 'NULL'}, ${location ? `'${location.replace(/'/g, "''")}'` : 'NULL'}, ${tagsStr ? `'${tagsStr.replace(/'/g, "''")}'` : 'NULL'}, ${featured}, 'published', NOW(), NOW(), ${dateStr ? `'${dateStr}'` : 'NULL'});`;
  
  statements.push(sql);
}

writeFileSync('/tmp/news_nocats.sql', statements.join('\n\n'));
console.log(`Generated ${statements.length} INSERT statements (no categories)`);
