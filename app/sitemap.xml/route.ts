...
const STATIC_ROUTES: Array<{
  ...
  { pathname: "/studio/apps/refro", priority: 0.6, changeFrequency: "monthly" },
  {
...
function getSitemapEntries(): MetadataRoute.Sitemap {
  ...
  const scenicEntries: MetadataRoute.Sitemap = getLocalScenicProjects().map(
    project => ({
  ...
  const renderingEntries: MetadataRoute.Sitemap =
    getLocalRenderingProjects().map(project => ({
  ...
  const experientialProjectEntries: MetadataRoute.Sitemap =
    getLocalExperientialProjects().map(project => ({
  ...
  const artifactEntries: MetadataRoute.Sitemap =
    getProjectArtifactCollections().map(collection => ({
      url: absoluteUrl(`/projects/artifacts/${collection.slug}`),
      lastModified: toLastModified(
        collection.year ? `${collection.year}-01-01` : null
      ),
      changeFrequency: "monthly",
      priority: 0.6,
    }));
